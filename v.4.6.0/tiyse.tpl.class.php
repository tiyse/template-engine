...
class tiyse{
	function __construct($setting = null){
		if($setting == true){ $this->setting = $setting; }else{ $this->setting = array('cache_dir'=>'/cache/','tpl_dir'=>'/','tpl_ext'=>'tpl'); }
	}
	function draw($draw){
		...
		$template = preg_replace_callback('#\{\$(.*?)\}#msi', function($matches){
			if(isset($this->assign[$matches[1]]) && !is_callable($matches[1])){
				return '<?php echo $'.$matches[1].'; ?>';
			}else if(is_callable($matches[1])){
				if(isset($this->assign[$matches[1]])){
					return '<?php echo '.$this->assign[$matches[1]].'; ?>';
				}else{
					return '<?php echo $'.$matches[1].'; ?>';
				}
			}else{
	...
	function assign($assign,$value = null){
		if(is_array($assign)){
			foreach($assign as $key => $value){ $this->assign[$key] = $value; }
		}else{ $this->assign[$assign] = $value; }
	}
	function compiler($template,$draw){
		...
		if(is_dir($_SERVER['DOCUMENT_ROOT'].$this->setting["cache_dir"]) && !file_exists($compilerfile)){
			...
			$compressed = gzcompress($secutiy.$template, 9);
	...
	function cache($draw){
		...
		if(is_dir($_SERVER['DOCUMENT_ROOT'].$this->setting["cache_dir"]) && file_exists($compilercache)){
			...
			extract($this->assign);
			return eval(" ?> ".gzuncompress(file_get_contents($compilercache))." <?php ");
		}
	}
}
?>