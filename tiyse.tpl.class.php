...
class tiyse{
	...
	function draw($draw,$cachetime = 0){
		...
		if(file_exists($tpl_file)){ include $tpl_file; }else{ echo "<code>Şablon mevcut değil: $tpl_file</code>"; }
		...
		$template = preg_replace_callback('#\{include="(.*?)"\}#msi',function($matches){
			$include = $_SERVER["DOCUMENT_ROOT"].$this->setting["tpl_dir"].$matches[1].'.'.$this->setting["tpl_ext"];
		...
		if(file_exists($cachefile) && (time() - $cachetime < filemtime($cachefile))){
			...
			if($cachetime == 0){
				...
				if(!is_dir($_SERVER['DOCUMENT_ROOT'].$this->setting["cache_dir"])){
					mkdir($_SERVER['DOCUMENT_ROOT'].$this->setting["cache_dir"]);
				}
				if(is_dir($_SERVER['DOCUMENT_ROOT'].$this->setting["cache_dir"])){
